...
.v-audio-player-control-panel
  ...
  template(v-if="!isPlayingAudio")
    svg(
      ...
      viewbox="0 0 24 24"
      fill="none"
      @click="clickedEvent(processEnum.PLAY)"
    )
  ...
  template(v-if="isPlayingAudio")
    svg(
      ...
      viewbox="0 0 24 24"
      fill="none"
      @click="clickedEvent(processEnum.PAUSE)"
    )
      rect(width="24" height="24" fill="white")
      path(
        fill-rule="evenodd"
  ...
  template(v-else-if="soundLevelType === soundLevelTypeEnum.LOW")
    svg(
      ...
      text-rendering="geometricPrecision"
      shape-rendering="geometricPrecision"
      @click="resetSoundLevel"
    )
      g
        path(
          stroke="null"
          d="m3.33418,5.64745l6.52346,0l5.39575,-5.29154c0.39987,-0.38926 1.04698,-0.39239 1.44662,-0.00313c0.20341,0.19174 0.30327,0.44721 0.30327,0.70586l0.00321,0l0,21.87388c0,0.55043 -0.45923,1.00049 -1.02819,1.00049c-0.28745,0 -0.54672,-0.11544 -0.73431,-0.30116l-5.33318,-4.21764l-6.57663,0c-0.89995,0 -1.71878,-0.35865 -2.3095,-0.93362c-0.5937,-0.57502 -0.96217,-1.37174 -0.96217,-2.24769l0,-7.40125c0,-0.87595 0.36847,-1.67271 0.9592,-2.24741c0.61489,-0.59873 1.44182,-0.9368 2.31248,-0.9368zm18.32027,3.3088c-0.16775,-0.48823 0.10283,-1.01696 0.60444,-1.18024c0.50161,-0.16327 1.04482,0.10004 1.21257,0.58828c0.33012,0.95402 0.48928,2.43499 0.46273,3.84327c-0.02579,1.38009 -0.23299,2.73615 -0.63392,3.53712c-0.23194,0.46397 -0.80708,0.65701 -1.28371,0.4313c-0.47668,-0.22571 -0.67501,-0.78552 -0.44312,-1.24948c0.27858,-0.55668 0.42485,-1.61966 0.44604,-2.74819c0.02311,-1.21785 -0.10226,-2.46318 -0.36502,-3.22205z"
          fill-rule="nonzero"
        )
...
import { processEnum, soundLevelTypeEnum } from '@/enums/index.ts'

export default defineComponent({
  name: 'VAudioPlayerControlPanel',
  setup() {
    const { increaseActiveAudioIndex, decreaseActiveAudioIndex } = inject('root')
    const { soundRef, playAudio, pauseAudio, changeSoundLevel, resetSoundLevel, isPlayingAudio, soundLevel, soundLevelType } =
      inject('operations')
